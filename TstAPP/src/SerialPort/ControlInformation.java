...
import com.example.SerialPort.Write;

public class ControlInformation {
    ParseControlJSON parseControlJSON=new ParseControlJSON();
    String ControlList[]=parseControlJSON.ReturnControlStatus();
    byte instruction1[] = new byte[] {0x01,0x06,0x00,(byte)0xB9,0x00,0x01,(byte)0x99,(byte)0xEF};//开机制冷
    byte instruction2[] = new byte[] {0x01,0x06,0x00,(byte)0xBA,0x00,0x01,0x69,(byte)0xEF};//开机制热
    byte instruction3[] = new byte[] {0x01,0x06,0x00, (byte) 0xBB,0x00,0x01,0x38,0x2F };//关机


    public void Controling(String[] str){
        System.out.print("控制状态：");
        for(int i=0;i<ControlList.length;i++){
            System.out.print(ControlList[i]+"--");
        }
        Write write=new Write();
        if(str[0].equals("Powered off!")&&ControlList[0].equals("开机")){
            write.ControlByModBus(instruction1);
        }

        if(ControlList[0].equals("关机")){
            write.ControlByModBus(instruction3);
        }else if(ControlList[0].equals("开机")&&Integer.parseInt(str[2])<Integer.parseInt(ControlList[2].substring(0,2))){
            write.ControlByModBus(instruction2);
        }else if(ControlList[0].equals("开机")&&Integer.parseInt(str[2])>Integer.parseInt(ControlList[2].substring(0,2))){
            write.ControlByModBus(instruction1);
        }
